...
processor = ColumnTransformer([('encoder', OneHotEncoder(), [0])], remainder='passthrough')

features = np.array(processor.fit(features), dtype=np.int)

p.predict(pred)

onehotencoder = OneHotEncoder(categories = "Department")
features = onehotencoder.fit_transform(features[0].reshape(1,-1)).toarray()
